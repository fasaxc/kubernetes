...
func (ep *endpointsInfo) Cleanup() {
	...
	if ep.refCount <= 0 && !ep.isLocal {
		...
		ep.hns.deleteEndpoint(ep.hnsID)
		ep.hnsID = ""
	}
...
func NewProxier(
	...
	if len(hnsNetworkName) == 0 {
		klog.V(3).Infof("Environment variable KUBE_NETWORK not initialized")
	...
	if err != nil {
		klog.Fatalf("Unable to find Hns Network specified by %s. Please check environment variable KUBE_NETWORK or network-name flag", hnsNetworkName)
		return nil, err
	}
	hnsNetworkInfo, err := getHnsNetworkInfo(hnsNetworkName)
	...
	for svcName, svcInfo := range proxier.serviceMap {
		...
		for _, ep := range proxier.endpointsMap[svcName] {
			...
			if newHnsEndpoint == nil {
				...
				if proxier.network.networkType == "Overlay" {
					networkV2, err := hns.getNetworkByName(hnsNetworkName)
					if err != nil {
						klog.Errorf("%v", err)
					}

					var providerAddress string
					for _, rs := range networkV2.remoteSubnets {
		...
		vip := "" // VIP has to be empty to automatically select the nodeIP in v1
		if proxier.network.networkType == "Overlay" {
			vip = proxier.nodeIP.String()
		}
		if svcInfo.nodePort > 0 {
			hnsLoadBalancer, err := hns.getLoadBalancer(
				...
				false,
				proxier.sourceVip,
				vip, // VIP has to be empty to automatically select the nodeIP
				Enum(svcInfo.protocol),
				uint16(svcInfo.targetPort),
